...
private _handle = [{
    ...
    if(_countdown <= 0) exitWith {
		[_this#1] CBAFUNC(removePerFrameHandler);
    ...
    if(_sound_interval > 0) then { //clusterfuck of if statements to get 3 intervals of different sounds
        if(round(_countdown) % _sound_interval == 0) then {
            [QACEGVAR(medical_feedback,forceSay3D), [_obj, _sound, 50], _targets] CBAFUNC(targetEvent);
        };
        ...
        if(_sound_interval isEqualTo 0) then {
            ...
            #endif
            [QACEGVAR(medical_feedback,forceSay3D) [_obj, QGVAR(timerEnd), 50], _targets] CBAFUNC(targetEvent);
            _obj setVariable [QGVAR(sound_interval), -1];
        };
    };

    _obj setVariable [QGVAR(time), _countdown,true];
}, 1, [_start_time,_end_time,_max_colors,_obj]] CBAFUNC(addPerFrameHandler);
